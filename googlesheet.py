...
def markatt(checklist,todate: str=today.strftime("%d/%m/%Y"),docname: str="MMM attendance"): #strftime to format dd/mm/yyyy
    ...
    for j in range(2):  #loops through all sheets
        ...
        print(f'Checking sheet{j+1}')
        row = len(sheet.row_values(1))
        collen = len(sheet.col_values(2))
        sheet.update_cell(1,row+1,todate)  #updating header(date)
        rang = sheet.get(f'B2:B{collen}')  #collen is length of the column in each sheet
